...
describe('Amazon search a product', async () => {
  //create test case
    it('Amazon search a product', async () => {
      ...
      await browser.pause(3000);
      await (await iipage.$type).click();
      await browser.pause(3000);
      await (await iipage.$dropdown).click();
      await browser.pause(2000);
      await (await iipage.$filter).click();
      await browser.pause(3000);
      await (await iipage.$pick).click();
      await expect(browser).toHaveUrlContaining("B09G95PQFF");
      await (await iipage.$additem).click();
      await browser.pause(2000);
      await (await iipage.$applecare).click();
      await expect(browser).toHaveTitleContaining("Apple iPhone 13 Pro Max (1TB) - Silver : Amazon.co.uk");
      await browser.pause(2000);
